...
function printCountryToHTML(country) {
  document.write(generateCountryHTML(country));
}

/**
 * Quiero pintar los paises cuyo name_es y name_en sean iguales
 * Entrada: Array de objetos que representan los paises
 * Salida: Undefined
 *
 * 1. Recorrer los paises
 *    a. Si name_es es igual a name_en, entonces lo mando a pintar al HTML
 *    b. Sino no hago nada
 */
...
/**
 * Devuelve la lista de paises que terminan por S,
 * en su nombre en español o en su nombre en ingles
 *
 * Entrada: Array de objetos paises
 * Salida: Array de objetos paises
 *
 * 1. recorrer los paises
 *     a. Si el pais su nombre_en acaba por S o su nombre_es acaba por ese
 *         lo añado a la lista final
 *     b. Si no no lo añado
 *
 */
function getCountriesEndsWithS(countries) {
...
/**
 * Devolver el país con el nombre en español más largo
 *
 * Entrada: Array de objetos países
 * Salida: Objeto de pais
 *
 * 0. Crear una variable donde voy guardando el país actual más largo
 * 1. recorremos los países
 *     a. Si el pais de la vuelta i tiene un nombre más largo que el que tenía ACUMULADO
 *         el nuevo país más largo que voy a ACUMULAR es el de la vuelta i
 *     b. Sino, mantengo el que tenía ACUMULADO
 */
function findLongestCountryByName(countries) {
  // let longestCountryFound = countries[0]; // paso 0
  // for(let i=0; i<countries.length; i++){ // paso 1
  //     if(longestCountryFound.name_es.length < countries[i].name_es.length){
  //         // paso 1.a
  //         longestCountryFound = countries[i]; // he encontrado un país más grande
  //     }
  // }
  // return longestCountryFound;

  return countries.reduce(
    (acc, v) => (acc.name_es.length < v.name_es.length ? v : acc),
