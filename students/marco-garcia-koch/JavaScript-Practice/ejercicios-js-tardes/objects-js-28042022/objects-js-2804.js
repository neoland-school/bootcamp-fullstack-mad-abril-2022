// 1- Crear un objeto que tenga dos propiedades ('demo'  de tipo string)  y ('hello' de tipo boolean). 
...
demoObject.hello = false;


//3- Crear un objeto que tenga dos propiedades , entre ellas una  de tipo number, 
// Imprime por pantalla el objeto y luego imprime por pantalla solo la propiedad de tipo number. 
// Repetir el proceso pero con propiedades que tengan 
// los tipos de datos string, boolean, null, undefined , array, function, objects.(easy).
...
const newObject = {
    ...
    race: null,
    married: undefined,
    hobbies: ['tenis', 'javascript', 'food'],
    salaryCalculation: function salary (income) {return income*3},
    familyMembers: {}
};

document.write(`<p>${newObject}</p>`);
document.write(`<p>${newObject.age}</p>`);
document.write(`<p>${newObject.name}</p>`);
document.write(`<p>${newObject.isOverAge}</p>`);
document.write(`<p>${newObject.race}</p>`);
document.write(`<p>${newObject.married}</p>`);
document.write(`<p>${newObject.hobbies}</p>`);
document.write(`<p>${newObject.salaryCalculation(1000)}</p>`);
document.write(`<p>${newObject.familyMembers}</p>`);


//4- Crea una función que dado un string con una hora (ej: 14:36:57) devuelva un objeto con la propiedad hora, 
...
//5-  Crea una función que imprima por pantalla el 'nombre: valor' de cada propiedad del objeto  
...
const printObjectProperties = object =>
    Object.entries(object).forEach(arr => document.write(`<p>${arr[0]} : ${arr[1]}</p>`));

printObjectProperties(marco);
...
// Una funcion que encuentre el pais con el nombre mas largo.
    ...
    function printCountryInHTML (country) {
        document.write(`
        <div>
            <div class="card-container">
                <p class="name-en">English Name: ${country.name_en}</p>
                <p class="name-es">Spanish Name: ${country.name_es}</p>
                <p class="dial-code">Dial Code: ${country.dial_code}</p>
                <p class="code">Country Code: ${country.code}</p>
            </div>
        </div>
        `);
    }

    function printOnlyCountrySameName (country) {
        country.forEach(c => {
            if (c.name_en === c.name_es) {
                printCountryInHTML(c)};
        })};
    ...
    function findCountryWithLongestName (country) {
        return country.reduce((acc, v) => v.name_es.length > acc.name_es.length ? v : acc), country[0];
    }


    printOnlyCountrySameName(countries);

    getOnlyCountryEndedS(countries);

    findCountryWithLongestName(countries);