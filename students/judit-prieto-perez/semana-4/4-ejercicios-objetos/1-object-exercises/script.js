 ...
 **/
  ...
  const  objeto_1= {
   ...
   objeto_1.hello= false

   console.log(objeto_1)

   /*Crear un objeto que tenga dos propiedades , entre ellas una  de tipo number, Imprime por pantalla el objeto y luego imprime 
   por pantalla solo la propiedad de tipo number. Repetir el proceso 
   pero con propiedades que tengan los tipos de datos string, boolean, null, undefined , 
...
demoObj= [{objeto_1}]
   document.write(`<p>Imprime el objeto por pantalla: </p>`)
...
document.write(`<p>Imprime los valore por pantalla: </p>`)
document.write 
(`Habitantes: ${city.population}
<p>País:  ${city.country}</p>
<p>Disponibilidlad: ${city.isAvaliable}</p>
<p>Array: ${city.demoArr}</p>
<p>Objeto: ${objeto_1.demo} <p/>
<p>Ejemplo de función: ${city.demoFn}</p>`);


/*Crea una función que dado un string con una hora (ej: 14:36:57) 
devuelva un objeto con la propiedad hora, minutos y segundos e imprimelo
 por pantalla.(medium)
 TIPO DE DATO QUE ENTRA: String
 Tipo de dato que SALE: OBJETO
1. Crear una función. La función tiene que convertir la hora en objeto.
          1.a. HORA-STRING---->HORA-ARRAY --CÓMO? ARRAY METHOD --SPLIT
          1.B. OBJETO CON ARRAY
...
return objeto
 ...
 console.log(hora('14:36:57'));


 /*Crea una función que imprima por pantalla el 'nombre: valor'
   de cada propiedad del objeto  (buscar mertodos de objeto ejmp Object Keys).(easy).*/

   let cars =  [
    {name:'toyota',year:2018, modelo: 'pegasus'},
    {name:'ford',year: 2014, modelo: 'lelo'},
    {name:'opel',year:2000, modelo: 'prilex'},
    {name:'ferrari',year: 2021, modelo:'triangle'},
  ]

//Object.keys(cars).forEach(year => console.log(`${year: ${cars[1]}`));


/*Una funcion que pinta en cards los paises cuyo nombre en ingles y español son iguales.
Una funcion de devuelva los paises cuyo nombre  termine con una S.
Una funcion que encuentre el pais con el nombre mas largo.

1. funcion que muestre todos los paises.


1. Es un array de objetos. 
En consola si ponemos console.log(countries)--> Array de 242 elementos

*/
 
document.write(`
<div class= country>
<h3 class= name_en>${country.name_en}<</h3>
<h3 class= name_en>${country.name_es}<</h3>
<p>${country.dial_code}<</p>

</div>`)
  
/*2. Funcion que devuelva los nombres que sean iguales

- Recorrer el array --> For Each
- Comprobar si  name_en === name_es
- Si se cumple la condición ---> print 
*/

function sameName(country){
 ...
 if(countries.name_en===countries.name_es) {
        ...
        <h3 class= name_en>${country.name_en}<</h3>
        <h3 class= name_en>${country.name_es}<</h3>
          <p>${country.dial_code}<</p></div>`)
  }
});
   }


  /*3. Una funcion de devuelva los paises cuyo nombre  termine con una S.
  
Dato que entra: Array de objetos.
Dato que sale: Array de objetos.

1. Nos pide recorrer el array. Entoces usamos también el forEach. //No lo usamos porque ya estamos "recorriendo" con el filter.
2. Si encuentra uno que termine con una S. Entonces --> PRINT
3. Buscar un método de array que sirva para "termina con una S"
Ej con "startswith("s")"
carList.filter(v => v.model.startsWith('s'))--> Nos puede servir. D

*/
function endsWithS(countries){
  countries.filter(v => v.endsWith('s'))
}

/*
countries.filter(
          (c) => c.name_en.endsWith("s") || c.name_es.endsWith("s")
        );
      }
...
console.log(endsWithS(countries))
const countries = [
    {
      "name_en": "Afghanistan",
      "name_es": "Afganistán",
