 ...
 **/
  ...
  const  objeto_1= {
   ...
   //1.A.Crear un objeto que tenga dos propiedades. Una tipo number.

  const cit ={
  population: 23.456, //tipo number
  country: 'Spain', 
  }
...
console.log
document.write(`${cit}`)
  document.write(`<p>Imprimirmos solo la propiedad tipo number: ${cit.population} habitantes.`)


  //Repetir el proceso pero con propiedades con tipos de datos string, boolean, null, etc
  const cities ={
    demoFn: () => {},
   isAvaliable: true,
   demoArr: [1,2,3],
...
/*
  function generateCountryHTML(country) {
    return `
         <article class="country__container">
            <h1 class="country__title">${country.name_en}</h1>
            <p class="country__dial--bold">${country.dial_code}</p>
         </article>
      `;
  }

<p>${city.country}</p>
<p>${city.isAvaliable}</p>
${city.demoArr}
${city.demoObj} ${city.demoFn});
...
function name (objeto){
  let name = Object.entries(objeto)
  return name
}
...
1. Funcion que encuentre paises cuyo nombre en inglés y español son iguales.
entran dos strings. 
expected output --> Albania, 
***Convertir el objeto y las propiedades en un array***
***Hacer una comparación de arrays***

countries.some(c => c.name_en === c.name_es);

function (a,b){
  let name= (name_en === name_es)
  return true
countries.some(c => c.name_en === c.name_es);

console.log(countries.some)
}



countriesList.filter(c => c.model.endsWith('S'))


const countries = []

const myArr = JSON.parse(countries);

console.log(countries)


countries = Object.create({}, { getFoo: { value: function() { return this.foo; } } });
countries.foo = '';
console.log(Object.entries(countries)); // [ ['foo', 'bar'] ]
